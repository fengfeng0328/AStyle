...
//       int x = 1;
			...
			if ( ( ( previousCommandChar == '{' && isPreviousBracketBlockRelated )
				...
				if ( bracketFormatMode == NONE_MODE ) {
					// if this text preceeds a broken bracket, break the line
					if ( isCharImmediatelyPostOpenBlock ) {
						// must be a broken bracket
						size_t i = formattedLine.find_first_not_of ( " \t" );
						
						if ( i != string::npos && formattedLine[i] == '{' ) {
							previousCommandChar = ' ';
							isInLineBreak = true;
							}
						}
			...
			if ( isPotentialHeader && !isInTemplate ) {
				...
				if ( newHeader != NULL ) {
					...
					// If in ATTACH or LINUX or STROUSTRUP bracket modes, attach closing headers (e.g. 'else', 'catch')
					// to their preceding bracket,
					// But do not perform the attachment if shouldBreakClosingHeaderBrackets is set
					// or if bracketIndent or blockIndent is set!
					if ( !shouldBreakClosingHeaderBrackets
					          && ! ( getBracketIndent() || getBlockIndent() )
					          && foundClosingHeader
					          && ( bracketFormatMode == ATTACH_MODE
					               || bracketFormatMode == LINUX_MODE
					               || bracketFormatMode == STROUSTRUP_MODE )
					          && ( shouldBreakOneLineBlocks || !isBracketType ( previousBracketType, SINGLE_LINE_TYPE ) )
					          && previousNonWSChar == '}' ) {
					...
					// unless bracketIndent or blockIndent is set!
			...
			// pad comma and semi-colon operators
			if ( currentChar == ',' || currentChar == ';' ) {
				char nextChar = ' ';
...
//    TRcomment(adjust);                  // trace macro
	...
	void ASFormatter::formatBrackets ( BracketType bracketType ) {
		...
		if ( currentChar == '{' ) {
			...
			// always break BREAK_MODE
			if ( bracketFormatMode == BREAK_MODE )
				breakBracket = true;
				
			// should a Linux or Stroustrup bracket be broken?
			if ( bracketFormatMode == LINUX_MODE || bracketFormatMode == STROUSTRUP_MODE ) {
			...
			if ( bracketFormatMode == NONE_MODE ) {
				if ( lineBeginsWith ( '{' ) )       // is opening bracket broken?
					...
					if ( peekNextChar() != '}'          // don't break {}
					          && peekChar != ' '          // break the line if a following statement
					          && peekChar != '/'
