#! /usr/bin/python
# Run AStyle to test the i18n component for Greek.
...
# WINDOWS
# The "Region and Language" setting should be set to Greek. 
...
# Greek codec - llisted in the documantation under "codecs (module)
if os.name == "nt":
	greek = "cp1253"	
else:
	greek = "utf_8"
...
# variables are converted to multi-byte Greek
subdir1   = u"/ΓΔΘΛ".encode(greek)
subdir1a  = u"/ΓΔΘΛ/αβγλ".encode(greek)
subdir1b  = u"/ΓΔΘΛ/ξπρσ".encode(greek)
subdir2   = u"/ΞΦΨΩ".encode(greek)
...
def test_recursive_3():
	...
	# filename and extension converted to multi-byte Greek
	fileNameJ = u"/ΠΖΣΧ".encode(greek)
	fileExtJ  = u".μςφ".encode(greek)
...
def test_recursive_suffix():
	...
	# must encode the Greek suffix to astyle
	excludes = []
	suffix = u".μθτη".encode(greek)
...
def verify_locale():
	...
	if os.name == "nt":
		if lang == "el_GR":
			print "Testing GREEK " + codepage
		else:
			print "LOCALE NOT SET TO GREEK: " + lang
	else:
		print "Testing GREEK " + codepage
