#! /usr/bin/python
# Run AStyle to test the i18n component for Russian.
...
# WINDOWS
# The "Region and Language" setting should be set to Russian. 
...
# Russian codec - llisted in the documantation under "codecs (module)
# in Windows Russian uses the same codepage as English
if os.name == "nt":
	russian = "cp1251"	
else:
	russian = "utf_8"
...
# variables are converted to multi-byte Russian
subdir1   = u"/АБВГ".encode(russian)
subdir1a  = u"/АБВГ/ДЕЁЖ".encode(russian)
subdir1b  = u"/АБВГ/ЗИЙК".encode(russian)
subdir2   = u"/ЛПФЦ".encode(russian)
...
def test_recursive_suffix():
	...
	# must encode the Russian suffix to astyle
	excludes = []
	suffix = u".фыюя".encode(russian)
...
def verify_locale():
	...
	if os.name == "nt":
		if lang == "ru_RU":
			print "Testing RUSSIAN " + codepage
		else:
			print "LOCALE NOT SET TO RUSSIAN: " + lang
	else:
		print "Testing RUSSIAN " + codepage
